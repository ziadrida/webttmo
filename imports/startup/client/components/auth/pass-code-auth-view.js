...
class PassCodeAuthView extends React.Component {
  ...
  handleSubmit = async (evt) => {
    evt.preventDefault();
    console.log('=> PassCodeAuthView handleSubmit ',this.state)
    console.log('=> PassCodeAuthView handleSubmit ',this.props)
    const {
      email,
      onBeforeHook,
      onClientErrorHook,
    ...
    if (fake ) {
      console.log('fake - return onSuccessHook')
      onSuccessHook({ faketoken });
      return
    }
    // Validate fields
    ...
    try {
      ...
      this.clearFields();
    //  let token = '123456'
      console.log('=> PassCodeAuthView handleSubmit before  ')
      onSuccessHook({ token });
    } catch (exc) {
      console.log(exc);
